...
def inject_global_data():
    ...
    cur.execute("SELECT COUNT(*) FROM prospects WHERE status='Qualified' AND outreach_status='Pending Approval'"); prospects = cur.fetchone()[0]
    conn.close()
    ...
    return {"nav_task_count":tasks,"nav_approval_count":ca+la,"nav_overdue_count":overdue,"nav_prospect_count":prospects,"operator":get_operator_status()}
...
def get_prospects():
    return fetchall("""SELECT id,business_name,industry,city,address,website,phone,maps_url,
...
@app.route("/prospects")
def prospects_page():
    return render_template("prospects.html", prospects=get_prospects())


@app.route("/prospect/<int:prospect_id>/approve", methods=["POST"])
def approve_prospect_outreach(prospect_id):
    conn=get_connection(); conn.execute("UPDATE prospects SET outreach_status='Approved' WHERE id=?", (prospect_id,)); conn.commit(); conn.close()
    log_activity("Prospect Outreach Approved", f"Outreach approved for prospect #{prospect_id}.")
    return redirect(url_for("prospects_page"))


@app.route("/prospect/<int:prospect_id>/reject", methods=["POST"])
def reject_prospect(prospect_id):
    conn=get_connection(); conn.execute("UPDATE prospects SET outreach_status='Rejected',status='Rejected' WHERE id=?", (prospect_id,)); conn.commit(); conn.close()
    log_activity("Prospect Rejected", f"Prospect #{prospect_id} rejected.")
    return redirect(url_for("prospects_page"))


@app.route("/prospect/<int:prospect_id>/convert", methods=["POST"])
def convert_prospect_to_lead(prospect_id):
    prospect=fetchone("SELECT business_name,phone,industry,city,fit_reason,converted_lead_id FROM prospects WHERE id=?", (prospect_id,))
    if not prospect or prospect[5]:
        return redirect(url_for("prospects_page"))

    name,phone,industry,city,reason,_ = prospect
    notes=f"Prospected by BizFlow. Industry: {industry or 'Unknown'}. City: {city or 'Unknown'}. Fit: {reason or ''}"
    conn=get_connection(); cur=conn.cursor()
    cur.execute("""INSERT INTO leads (name,email,phone,source,status,followup_status,pipeline_stage,lead_value,notes,lead_score)
                   VALUES (?,NULL,?,'Prospecting','New','Not Drafted','New Lead',0,?,60)""", (name,phone,notes))
    conn.commit(); conn.close()

    lead=fetchone("SELECT id FROM leads WHERE name=? AND source='Prospecting' ORDER BY id DESC LIMIT 1", (name,))
    lead_id=lead[0] if lead else None
    conn=get_connection(); conn.execute("UPDATE prospects SET status='Converted',converted_lead_id=? WHERE id=?", (lead_id,prospect_id)); conn.commit(); conn.close()
    log_activity("Prospect Converted", f"Prospect #{prospect_id} converted into a BizFlow lead.")
    return redirect(url_for("prospects_page"))
